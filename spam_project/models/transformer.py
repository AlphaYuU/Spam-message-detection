...
from spam_project.model_catalog import DEFAULT_TRANSFORMER_MAX_LENGTH, PROJECT_ROOT
from spam_project.models.types import PredictionResult
...
def predict_transformer_result(
    model_dir,
    message: str,
    display_name: str,
    max_length: int = DEFAULT_TRANSFORMER_MAX_LENGTH,
) -> PredictionResult:
    prediction = predict_transformer_spam(model_dir, message, max_length=max_length)
...
def predict_transformer_spam(model_dir, message, max_length=DEFAULT_TRANSFORMER_MAX_LENGTH):
    try:
...
def _predict_transformer_spam_with_helper(model_dir, message, max_length):
    ...
    if helper_python is None:
        raise MissingTransformerDependencies(
            ...
            "C:\\ProgramData\\anaconda3\\envs\\py310\\python.exe main.py."
        )
    ...
    hf_home = PROJECT_ROOT / "local_artifacts" / "huggingface" / "models"
    if hf_home.exists():
        env["HF_HOME"] = str(hf_home)
        env.pop("TRANSFORMERS_CACHE", None)

    helper_script = PROJECT_ROOT / "transformer_spam_model.py"
    completed = subprocess.run(
        [str(helper_python), str(helper_script), "--predict-json"],
...
def _main():
    ...
    result = _predict_transformer_spam_local(
        payload["model_dir"],
        payload["message"],
        int(payload.get("max_length", DEFAULT_TRANSFORMER_MAX_LENGTH)),
    )
    print(
        json.dumps(
            {
